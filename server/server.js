...
io.on("connection", function(socket){
	...
	socket.on('client-create', function(data) {
				...
				io.to(socket.id).emit('server-error-msg', msg);
					...
					roomls.forEach(function(value){
						if(value.roomid == data.roomid){
							...
							io.to(value.roomid).emit('server-send-room-data', value);
							if(value.playing == 1){
								io.to(value.player1id).emit('server-send-play-signal');
								io.to(value.player2id).emit('server-send-stop-signal');
							}else if(value.playing == 2){
								io.to(value.player2id).emit('server-send-play-signal');
								io.to(value.player1id).emit('server-send-stop-signal');
							}
							return;
						}
					});
  	...
  	socket.on('client-choose-img', function(data) {
  		...
  		if(imgchooselist.length == 0){
  			...
  			roomls.forEach(function(values){
			    if(values.roomid == playerProfile.room){
			    	if(values.playing == 1){
						io.to(values.player1id).emit('server-send-play-signal');
					}else if(values.playing == 2){
						io.to(values.player2id).emit('server-send-play-signal');
					}
			    }
			});
  			...
  			if(newchoose.imgo.img == imgchooselist[0].imgo.img){ //hinh 2 giong hinh 1	
  				roomls.forEach(function(value,index){
		    		if(value.roomid == playerProfile.room){
		    			...
		    			//neu da mo het
		    			if(value.player2point + value.player1point == 8){
  	...
  	socket.on('view-id', function(data) {
  		console.log(playerProfile.name);
  	});
});